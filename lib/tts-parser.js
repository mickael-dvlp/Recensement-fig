 ...
 * @param {string} texte - Texte exporté depuis un army builder au format TTS
 ...
 * Les options sont triées pour que l'ordre de saisie n'affecte pas la correspondance.
 */
...
/**
 * Résout un nom TTS vers un ID de figurine de l'app via la table de mapping.
 * Retourne "_skip" si la figurine doit être ignorée (ex: hors-jeu, duplicat connu),
 * null si aucune correspondance n'existe (à afficher dans la section "non reconnues").
 *
 * Ordre de recherche :
 * 1. Correspondance exacte nom + options triées
 * 2. Repli sur le nom seul (sans options)
 * 3. Si parenthèses dans le nom (ex: "Nazgul (Khamul)"), réessaie les deux étapes sans
 *    — pour gérer les notes de contexte TTS type "King of Men (Cirion)"
 * @param {string} nom
 * @param {string[]} options
 * @param {Record<string,string>} mapping - Objet de correspondance clé→ID chargé depuis tts-mapping.json
 * @returns {string|null}
 */
