...
export function parseTTS(texte) {
  ...
  for (const ligne of lignes) {
    ...
    // Supprimer les notes entre parenthèses ex : "King of Men (Cirion Intendant)" → "King of Men"
    const nom = match[2].trim().replace(/\s*\([^)]*\)/g, "").trim();
 ...
 * 1. Essaie la correspondance exacte (nom + options triées)
 * 2. Repli sur le nom seul (utile pour les héros dont les options sont des upgrades virtuels)
 */
