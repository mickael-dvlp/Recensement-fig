...
export async function creerMemo(uid, { titre, texte, type = "memo", figurines = [] }) {
  const colRef = collection(db, "utilisateurs", uid, "memos");
  const countSnap = await getCountFromServer(colRef);
  if (countSnap.data().count >= LIMITE_MEMOS) throw new Error("limite_memos_atteinte");
  const newRef = doc(colRef);
  const batch = writeBatch(db);
  batch.set(newRef, { titre, texte, type, figurines, creeLe: serverTimestamp(), ordre: -Date.now() });
...
// La vérification du compteur ici est une sécurité côté client ; la règle Firestore
// rejette de toute façon le batch si compteurCustom ≥ 200.
// L'upload Storage précède le batch Firestore : si le batch échoue, le fichier
// reste orphelin dans Storage (acceptable, nettoyage possible a posteriori).
export async function creerFigurineCustom(uid, { nom, faction, section, file }) {
  const colRef = collection(db, "utilisateurs", uid, "figurinesCustom");
  const countSnap = await getCountFromServer(colRef);
  ...
  const batch = writeBatch(db);
  batch.set(doc(db, "utilisateurs", uid, "figurinesCustom", id), {
