...
// ---- COLLECTION APPROFONDIE (mode d'affichage) ----

// Bascule Collection Normale / Approfondie : simple préférence sur le profil, synchronisée
// entre appareils. Les guerriers ayant des `variantesDetaillees` (data/factions/*.js) sont
// alors affichés sculpt par sculpt sur les pages figurines/faction — chaque sculpt est une
// entrée d'inventaire normale comme une autre (même sous-collection "inventaire", id =
// variante.id), la matière étant une donnée fixe du catalogue, pas un choix de l'utilisateur.
// setDoc({merge:true}) plutôt que updateDoc : les comptes invités (anonymes) n'ont pas
// de document profil (creerProfil n'est appelé qu'à l'inscription/connexion Google), donc
// updateDoc échouerait avec "No document to update" pour eux.
export async function basculerModeCollection(uid, mode) {
  await setDoc(doc(db, "utilisateurs", uid), { modeCollection: mode }, { merge: true });
}
